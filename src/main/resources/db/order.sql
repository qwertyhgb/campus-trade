...
CREATE TABLE IF NOT EXISTS `order` (
                         ...
                         -- SQL：SELECT * FROM `order` WHERE buyer_id=? ORDER BY create_time DESC
                         ...
                         -- SQL：SELECT * FROM `order` WHERE seller_id=? ORDER BY create_time DESC
                         -- 查询频率：极高（卖家每次打开“我卖出的”页面都会触发）
                         -- 最左前缀：
                         --   ✅ WHERE seller_id=? ORDER BY create_time DESC → 完全命中
                         ...
                         -- SQL：SELECT * FROM `order` WHERE status=0 AND create_time < ?
